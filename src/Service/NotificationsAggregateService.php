...
namespace BlackCat\Database\Packages\Notifications\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Notifications\Dto\NotificationDto;
use BlackCat\Database\Packages\Notifications\Mapper\NotificationDtoMapper;
 ...
 */
final class NotificationsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private NotificationRepository $notificationRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Notifications\ModuleException("Record {$idStr} not found for locking.");
            }
