...
use BlackCat\Database\Packages\Notifications\Dto\NotificationDto as Dto;
use BlackCat\Database\Packages\Notifications\Mapper\NotificationDtoMapper as RowMapper;
...
final class NotificationRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Notifications\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Notifications\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Notifications\Dto\NotificationDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Notifications\Dto\NotificationDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Notifications\Mapper\NotificationDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Notifications\Dto\NotificationDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Notifications\Dto\NotificationDto|null {
