...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'tenant_id', 'user_id', 'channel', 'template', 'payload', 'status', 'retries', 'max_retries', 'next_attempt_at', 'scheduled_at', 'sent_at', 'error', 'last_attempt_at', 'locked_until', 'locked_by', 'priority', 'created_at', 'updated_at', 'version' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'channel', 'template', 'status', 'error', 'locked_by' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'tenant_id', 'user_id', 'channel', 'template', 'status', 'retries', 'max_retries', 'next_attempt_at', 'scheduled_at', 'sent_at', 'error', 'last_attempt_at', 'locked_until', 'locked_by', 'priority', 'created_at', 'updated_at', 'version' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\Notifications\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\Notifications\Definitions::softDeleteColumn();
