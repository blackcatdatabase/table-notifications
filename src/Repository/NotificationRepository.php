...
namespace BlackCat\Database\Packages\Notifications\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Notifications\Definitions;
use BlackCat\Database\Packages\Notifications\Criteria;
use BlackCat\Database\Packages\Notifications\ContractRepository;

final class NotificationRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO notifications (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO notifications (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO notifications (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE notifications SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE notifications SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM notifications WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE notifications SET $set WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM notifications WHERE $whereSql LIMIT 1";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM notifications t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM notifications t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM notifications WHERE id = :id FOR UPDATE"
            : "SELECT * FROM notifications WHERE id = :id FOR UPDATE";
